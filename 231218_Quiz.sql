...
SELECT E.FIRST_NAME 이름,
       NVL(D.DEPARTMENT_NAME, '없음') 소속부서명,
       NVL(M.FIRST_NAME, '없음') 관리자명
...
SELECT E.FIRST_NAME, M.CNT
FROM (SELECT MANAGER_ID, COUNT(*) CNT
      FROM EMPLOYEES
      WHERE MANAGER_ID IS NOT NULL
      GROUP BY MANAGER_ID) M, EMPLOYEES E
...
SELECT D.DEPARTMENT_NAME, M.FIRST_NAME, C.CNT, C.AVG_SALARY
...
--Neena와 동일한 급여 모든 사원의 이름과 입사일 급여를 조회하기, 결과에 Neena는 포함시키지 않기
